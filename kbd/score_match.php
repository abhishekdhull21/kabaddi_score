...
<?php if ($status == 2) {
	echo "<div class='card-panel center-align ' ><h2 class='center-align blue-text text-darken-2' >Match ended</h2>
		<a href='select_match.php?rdr=score_match' ><button class='center-align  btn'> New Match </button></a>
	</div>";
}
else
{
...
<?php
	if (isset($_GET['submit'])) {
		...
		if ($status  == 0) 
		{
			$sql= "UPDATE matchs SET status = 1 where match_id = $match_id";
			$obj->update($sql);
		}
		elseif(isset($_GET['finish']))
			if($_GET['finish'] == 1)
			{
				$sql= "UPDATE matchs SET status = 2 where match_id = $match_id";			
				$obj->update($sql);
			}
